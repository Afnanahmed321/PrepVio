...
const PlaylistManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.data?.message && err.response.data.message.includes('E11000')) {
        setError('This video/playlist is already associated with the selected course.');
      } else {
        setError(err.message || `Failed to ${modalType} content`);
      }
    } finally {
      setLoading(false);
    }
  ...
  // Get channels for the selected course or all channels if no course is selected
              ...
              playlists.map(playlist => (
                <tr key={playlist._id}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500">
                    <a href={playlist.link} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                      {playlist.link}
                    </a>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{playlist.channelId?.name || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{playlist.courseId?.name || 'N/A'}</td>
        ...
        <Modal title={`${modalType === 'add' ? 'Add' : 'Edit'} Content`} onClose={handleCloseModal}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end space-x-2 mt-4">
              <button type="button" onClick={handleCloseModal} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition-colors">
                Cancel
              </button>
