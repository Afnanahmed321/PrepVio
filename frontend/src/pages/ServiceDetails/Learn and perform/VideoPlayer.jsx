...
  const [shownQuizzes, setShownQuizzes] = useState(new Set());
  
  // ✅ NEW: Refs for player and interval
...
  // ✅ NEW: Function to save progress to localStorage
  const saveProgress = async (time) => {
    if (selectedVideoId && time) {
      localStorage.setItem(`video_progress_${selectedVideoId}`, time);
      console.log(`Progress saved locally: ${time}s`);
    }
  };

  // ✅ NEW: Function to load progress from localStorage
  const loadProgress = () => {
...
  const handleVideoSelect = (video) => {
    // Stop the previous save interval
...
    // Immediately load and seek to the saved time
    const startTime = loadProgress();
...
      clearInterval(saveIntervalRef.current);
    }
    // Start new interval to save progress every 5 seconds
...
    }
    if (playerRef.current && selectedVideoId) {
      const finalTime = await playerRef.current.getCurrentTime();
      saveProgress(Math.floor(finalTime));
    }
  };
...
      const quizTime = Math.floor(q.timestamp);
      const isDue = Math.abs(quizTime - currentTime) <= 1 && !shownQuizzes.has(q._id);
      return isDue;
    });
    
    if (dueQuizzes.length > 0) {
      setQuizQueue((prev) => [...prev, ...dueQuizzes]);
      setShownQuizzes((prev) => new Set([...prev, ...dueQuizzes.map(q => q._id)]));
    }
  };

  useEffect(() => {
    // This effect runs when the component mounts or selectedVideoId changes
    // It is primarily for cleaning up the interval when the component unmounts
    return () => {
      handlePause(); // This will save the last timestamp and clear the interval
    };
  }, [selectedVideoId]);
